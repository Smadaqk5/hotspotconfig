"""
Pesapal integration for provider subscriptions
"""
import requests
import json
import base64
...
class ProviderSubscriptionService:
    ...
    def __init__(self):
        self.pesapal = PesapalProviderAPI()
    
    def create_subscription_order(self, provider, subscription_plan):
        """Create Pesapal order for provider subscription"""
        try:
            # Get access token
            ...
            payment = Payment.objects.create(
                provider=provider,
                type='pesapal_provider_subscription',
                amount=subscription_plan.price,
                currency='KES',
                external_txn_id=order_id,
                status='pending',
                provider_payload={'order_id': order_id, 'ipn_id': ipn_id}
            )
    ...
    def handle_pesapal_webhook(self, webhook_data):
        """Handle Pesapal webhook for payment confirmation"""
        try:
            from .models import Payment
            from subscriptions.models import ProviderSubscription
            
            # Extract order information
            order_id = webhook_data.get('order_tracking_id')
            payment_status = webhook_data.get('payment_status')
            payment_method = webhook_data.get('payment_method')
            payment_account = webhook_data.get('payment_account')
            
            if not order_id:
                return False, "No order tracking ID provided"
            ...
            # Update payment
            payment.provider_payload = webhook_data
            ...
            if payment_status == 'COMPLETED':
                ...
                logger.info(f"Payment status: {payment_status} for order {order_id}")
                return True, f"Payment status: {payment_status}"
                
        except Exception as e:
            logger.error(f"Failed to handle Pesapal webhook: {e}")
            return False, str(e)
    ...
    def check_subscription_status(self, subscription):
        ...
        try:
            ...
            status_data = self.pesapal.get_transaction_status(access_token, subscription.order_id)
            if not status_data:
